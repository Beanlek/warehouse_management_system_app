...
import 'package:warehouse/presentation/views/page_transfer_inout/layout/transfer_inout_detail.dart';
import 'package:warehouse/routes/routes.dart';
import 'package:warehouse/shared_preference/token.dart';
import 'package:warehouse/utils/utils.dart';

const String TYPE = TRANSFER_INOUT;

class TransferInOutListing extends StatefulWidget {
  const TransferInOutListing({super.key});

  @override
  _TransferInOutListingState createState() => _TransferInOutListingState();
}

class _TransferInOutListingState extends State<TransferInOutListing> {
  List<Map<String, dynamic>> transferInOuts = [];
  ...
  List<String> filtersTransferType = [
    TRANSFER_INOUT,
    TRANSFER_IN,
    TRANSFER_OUT,
  ];
  ...
  String selectedFilterTransferType = TRANSFER_INOUT;
  ...
  Future<void> fetchAPI(String? token) async {
    ...
    Map<String, String> params = {
      ...
      'type': selectedFilterTransferType,
    ...
    if (response.statusCode == 500) {
      ...
      try {
        ...
        setState(() {
          transferInOuts = List<Map<String, dynamic>>.from(wms_van_ids).toList();
        });
      } catch (e) {
  ...
  Future<void> _refreshData() async {
    setState(() {
      transferInOuts.clear(); // Clear the existing data
    });
  ...
  void _onSearchSubmitted(String query) {
    setState(() async {
      ...
      // Filter transferInOuts based on search text
      transferInOuts = transferInOuts.where((_transferOuts) {
              ...
              child: Container(
                ...
                child: Align(
                  ...
                  child: DropdownButton<String>(
                    padding: EdgeInsets.only(right: 12, left: 12),
                    isExpanded: true,
                    value: selectedFilterTransferType,
                    items: filtersTransferType
                        .map(
                          (filter) => DropdownMenuItem<String>(
                            alignment: AlignmentDirectional.centerEnd,
                            value: filter,
                    ...
                    onChanged: (filter) async {
                      setState(
                        () {
                          selectedFilterTransferType = filter!;
                          _currentPage = 0;
                          _paginatorController.currentPage = 0;
                          transferInOuts.clear();
                        },
  ...
  Widget _buildListView(String _filter) {
    ...
    List<Map<String, dynamic>> filteredTransferIns =
        transferInOuts.where((_transferOuts) {
      // final vanId = _transferOuts['van_id'].toString().toLowerCase();
      final _transferOutsId = _transferOuts['id'].toString().toLowerCase();
      final site = _transferOuts['site_id'].toString().toLowerCase();
      final recordType = _transferOuts['record_type'].toString().toLowerCase();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final transferId = transferOut['id'] ?? 'null';
                ...
                Expanded(
                  child: SizedBox(
                    child: ListTile(
                      ...
                      onTap: () async { debugPrint(transferId);
