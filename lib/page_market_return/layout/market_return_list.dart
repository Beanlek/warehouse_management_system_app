...
import 'package:warehouse/page_market_return/layout/market_return_detail.dart';
...
String titleCheck(String _recordType) {
  ...
  const String TYPE = 'market_return';

class MarketReturnListing extends StatefulWidget {
  const MarketReturnListing({super.key});

  @override
  _MarketReturnListingState createState() => _MarketReturnListingState();
}
...
class _MarketReturnListingState extends State<MarketReturnListing> {
  ...
  Future<void> fetchAPI(String? token) async {
    ...
    if (response.statusCode == 500) {
      ...
      try {
        ...
        setState(() {
          marketReturns = List<Map<String, dynamic>>.from(wms_van_ids).toList();
        });
      } catch (e) {
  ...
  Future<void> _refreshData() async {
    setState(() {
      marketReturns.clear(); // Clear the existing data
    });
  ...
  void _onSearchSubmitted(String query) {
    setState(() async {
      ...
      // Filter marketReturns based on search text
      marketReturns = marketReturns.where((_marketReturns) {
        // final vanId = _marketReturns['van_id'].toString().toLowerCase();
        final marketReturnsId = _marketReturns['id'].toString().toLowerCase();
        final site = _marketReturns['site_id'].toString().toLowerCase();
        final recordType = _marketReturns['record_type'].toString().toLowerCase();
        final status = _marketReturns['status'].toString().toLowerCase();
        final searchText = _searchFieldController.text.toLowerCase();

        return //vanId.contains(searchText) ||
            marketReturnsId.contains(searchText) ||
  ...
  Widget _buildListView(String _filter) {
    ...
    List<Map<String, dynamic>> filteredMarketReturns =
        marketReturns.where((_marketReturns) {
      // final vanId = _marketReturns['van_id'].toString().toLowerCase();
      final marketReturnsId = _marketReturns['id'].toString().toLowerCase();
      final site = _marketReturns['site_id'].toString().toLowerCase();
      final recordType = _marketReturns['record_type'].toString().toLowerCase();
                ...
                Expanded(
                  child: SizedBox(
                    child: ListTile(
                      ...
                      onTap: () async { print(marketReturnId);
                        bool tempRefresh = false;
                        tempRefresh = await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => MarketReturnDetailView(
